...
class Spider(BaseSpider):
    ...
    # ========================== 核心辅助解密：突破JS Token防火墙 ==========================
    def _encrypt_token(self, string):
        chars = "PXhw7UT1B0a9kQDKZsjIASmOezxYG4CHo5Jyfg2b8FLpEvRr3WtVnlqMidu6cN"
        char_len = len(chars)
        result = []
        for char in string:
            idx = chars.find(char)
            r1 = chars[random.randint(0, char_len - 1)]
            r2 = chars[random.randint(0, char_len - 1)]
            middle = char if idx == -1 else chars[(idx + 3) % char_len]
            result.extend([r1, middle, r2])
    ...
    def searchContent(self, key, quick, pg="1"):
        try:
            ...
            # 使用 Session 保持 Cookie 会话，对于绕过防火墙至关重要
            session = requests.Session()
            session.headers.update(self.headers)
            
            # 第一页 POST，翻页 GET（注意去斜杠规则）
            if str(pg) == "1":
                url = f"{self.host}/vodsearch.html"
                rsp = session.post(url, data={"wd": key, "submit": ""}, timeout=8, verify=False)
            else:
                url = f"{self.host}/vodsearch{urllib.parse.quote(key)}/page/{pg}.html"
                rsp = session.get(url, timeout=8, verify=False)
                
            rsp.encoding = "utf-8"
            html = rsp.text
            
            # 自动探测并强突 JS Token 防火墙
            if "v-thumb" not in html and "var token = encrypt" in html:
                token_match = re.search(r'var token = encrypt\("([^"]+)"\)', html)
                if token_match:
                    raw_token = token_match.group(1)
                    value_enc = self._encrypt_token(url)
                    token_enc = self._encrypt_token(raw_token)
                    
                    robot_url = f"{self.host}/robot.php"
                    robot_data = f"value={urllib.parse.quote(value_enc)}&token={urllib.parse.quote(token_enc)}"
                    
                    session.headers.update({"Content-Type": "application/x-www-form-urlencoded"})
                    session.post(robot_url, data=robot_data, timeout=8, verify=False)
                    
                    # 突破成功后，携带验证过的 Cookie 重发搜索请求
                    session.headers.update({"Content-Type": "application/x-www-form-urlencoded" if str(pg) == "1" else "*/*"})
                    if str(pg) == "1":
                        rsp = session.post(url, data={"wd": key, "submit": ""}, timeout=8, verify=False)
                    else:
                        rsp = session.get(url, timeout=8, verify=False)
            ...
            return {"list": [{"vod_id": "", "vod_name": f"搜索突破异常: {str(e)}", "vod_pic": "", "vod_remarks": "联系维护"}]}
    # ==========================================================================================
