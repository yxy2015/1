...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        try:
            ...
            # 查找分页元素
            pagination = doc.xpath('//div[@class="mypage"]/a[contains(@href, "/page/")]/@href')
            if pagination:
                # 找到最后一个分页链接
                last_page_url = pagination[-1] if pagination else ""
                ...
                # 尝试从文本中提取分页信息
                page_text = doc.xpath('//div[@class="mypage"]//text()')
                if page_text:
                    page_match = re.search(r'(\d+)/(\d+)页', ''.join(page_text))
                    if page_match:
