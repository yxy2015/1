...
class Spider(Spider):
    ...
    def init(self, extend):
        ...
        self.proxy_base = 'https://down.nigx.cn/proxy?single=true&url='
        self.headers = {
    ...
    def _get_without_proxy(self, url, headers=None):
        """不使用代理发送请求的辅助方法"""
        if headers is None:
            headers = self.headers
        try:
    ...
    def detailContent(self, did):
        ...
        try:
            res = self._get_with_proxy(url, headers=h)  # 详情页走代理
            if res is None or res.status_code != 200:
                return {'list': [], 'parse': 0, 'jx': 0, 'msg': f'请求失败: {res.status_code if res else "网络错误"}'}
            ...
            video_list.append(
                {
                    ...
                    'vod_content': '',
                    'vod_play_from': 'Rou',
                    'vod_play_url': f'1${play_url}',  # 播放地址会通过 playerContent 走代理
                    'msearch': True
                }
            )
    ...
    def searchContent(self, key, quick, page='1'):
        ...
        try:
            res = self._get_without_proxy(url)  # 搜索不走代理
            if res is None or res.status_code != 200:
                return {'list': [], 'parse': 0, 'jx': 0, 'msg': f'请求失败: {res.status_code if res else "网络错误"}'}
            root = etree.HTML(res.text.encode('utf-8'))
            data_list = root.xpath('//div[@class="aspect-video relative"]/a')
    ...
    def playerContent(self, flag, pid, vipFlags):
        # 播放地址走代理
        proxy_play_url = self.proxy_base + urllib.parse.quote(pid)
        return {
            'url': proxy_play_url,  # 代理后的播放地址
            'header': self.headers,
            'parse': 0,
            'jx': 0
        }
